import type { SyncMutation } from "@kharon/domain";
import { request, requireData } from "./http";
import type {
  ApiEnvelope,
  AutomationJobEntry,
  EscrowRecord,
...
export const apiClient = {
  ...
  async session(): Promise<PortalSession> {
    const result = await request<PortalSessionState>("/api/v1/auth/session", { method: "GET" });
  ...
  async authConfig(): Promise<PortalAuthConfig> {
    const result = await request<PortalAuthConfig>("/api/v1/auth/config", { method: "GET" });
  ...
  async listJobs() {
    const result = await request<Array<Record<string, unknown>>>("/api/v1/jobs", { method: "GET" });
    return result.data ?? [];
  },
  async getJob(jobid: string) {
    return request<Record<string, unknown>>(`/api/v1/jobs/${jobid}`, { method: "GET" });
  },
  ...
  async requestSchedule(jobid: string, slot: { start_at: string; end_at: string }, timezone: string, rowVersion: number) {
    return request<Record<string, unknown>>("/api/v1/schedules/request-slot", {
      method: "POST",
      body: JSON.stringify({ job_id: jobid, preferred_slots: [slot], timezone, notes: "", row_version: rowVersion })
    });
  },
  async confirmSchedule(requestid: string, startAt: string, endAt: string, technicianid: string, rowVersion: number, options?: { job_id?: string }) {
  ...
  async reschedule(
    ...
    endAt: string,
    rowVersion: number,
    options?: { job_id?: string; technician_id?: string; request_id?: string; calendar_event_id?: string }
  ) {
  ...
  async generateDocument(jobid: string, documentType: "jobcard" | "service_report" | "certificate", tokens: Record<string, string> = {}) {
    return request<Record<string, unknown>>("/api/v1/documents/generate", {
      method: "POST",
      body: JSON.stringify({ job_id: jobid, document_type: documentType, tokens })
    });
  },
  ...
  async history(jobid?: string) {
    ...
    return request<Array<Record<string, unknown>>>(`/api/v1/documents/history${suffix}`, { method: "GET" });
  },
  async dispatchContext(jobid: string): Promise<PortalDispatchContext> {
    const result = await request<PortalDispatchContext>(`/api/v1/workspace/dispatch-context?job_id=${encodeURIComponent(jobid)}`, { method: "GET" });
  ...
  async listPeople(): Promise<PeopleDirectoryEntry[]> {
    const result = await request<PeopleDirectoryEntry[]>("/api/v1/workspace/people", { method: "GET" });
    return result.data ?? [];
  },
  async adminHealth() {
    return request<Record<string, unknown>>("/api/v1/admin/health", { method: "GET" });
  },
  async adminAudits() {
    return request<Array<Record<string, unknown>>>("/api/v1/admin/audits", { method: "GET" });
  },
  async adminAutomationJobs() {
    return request<Array<Record<string, unknown>>>("/api/v1/admin/automation-jobs", { method: "GET" });
  },
  async listAutomationJobs(): Promise<AutomationJobEntry[]> {
    const result = await request<AutomationJobEntry[]>("/api/v1/admin/automation-jobs", { method: "GET" });
    return result.data ?? [];
  },
  async retryAutomation(automationJobid: string) {
    return request<Record<string, unknown>>(`/api/v1/admin/retries/${automationJobid}`, { method: "POST" });
  },
  async getUpgradeWorkspaceState(): Promise<UpgradeWorkspaceState> {
    const result = await request<UpgradeWorkspaceState>("/api/v1/workspace/upgrade/state", { method: "GET" });
    return requireData(result, "Upgrade state request succeeded but no state payload was returned.");
  },
  async createFinanceQuote(payload: { job_id: string; client_id: string; description: string; amount: number }): Promise<FinanceQuoteRecord> {
    const result = await request<FinanceQuoteRecord>("/api/v1/workspace/upgrade/quotes", { method: "POST", body: JSON.stringify(payload) });
  ...
  async reconcileInvoice(invoice_id: string): Promise<FinanceInvoiceRecord> {
    const result = await request<FinanceInvoiceRecord>(`/api/v1/workspace/upgrade/invoices/${encodeURIComponent(invoice_id)}/reconcile`, { method: "POST" });
  ...
  async getEscrowByDocument(document_id: string): Promise<EscrowRecord | null> {
    const result = await request<EscrowRecord | null>(`/api/v1/workspace/upgrade/escrow/${encodeURIComponent(document_id)}`, { method: "GET" });
  ...
  async rebuildUpgradeAnalytics(): Promise<{ debtors: number; statements: number }> {
    const result = await request<{ debtors: number; statements: number }>("/api/v1/workspace/upgrade/analytics/rebuild", { method: "POST" });
  ...
  async syncPush(mutations: SyncMutation[]): Promise<ApiEnvelope<SyncPushResult>> {
    return request<SyncPushResult>("/api/v1/sync/push", { method: "POST", body: JSON.stringify({ mutations }) });
  },
  ...
  async syncPull(since: string): Promise<SyncPullPayload> {
    const result = await request<SyncPullPayload>(`/api/v1/sync/pull?since=${encodeURIComponent(since)}`, { method: "GET" });
    return requireData(result, "Sync pull succeeded but no payload was returned.");
  },
  async schemaDrift(): Promise<SchemaDriftPayload> {
    const result = await request<SchemaDriftPayload>("/api/v1/workspace/schema-drift", { method: "GET" });
  ...
  async opsIntelligence(): Promise<OpsIntelligencePayload> {
    const result = await request<OpsIntelligencePayload>("/api/v1/workspace/ops-intelligence", { method: "GET" });
