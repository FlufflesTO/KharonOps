...
export async function enqueueMutation(item: OfflineQueueItem): Promise<void> {
  const database = await db();
  await database.put(STORE_NAME, item);
}

export async function listQueuedMutations(): Promise<OfflineQueueItem[]> {
  const database = await db();
  const all = await database.getAll(STORE_NAME);
  return all.sort((a, b) => a.created_at.localeCompare(b.created_at));
}

export async function removeQueuedMutations(mutationIds: string[]): Promise<void> {
  const database = await db();
